...
function custom_get_additional_fees_settings() {
    ...
    $additional_fees_settings = array(
        ...
        'priority_delivery' => array(
            'name' => __('Entrega prioritária', 'nutrisslim-suiteV2'),
            'type' => 'number',
        ...
        'package_insurance' => array(
            'name' => __('Seguro de encomenda', 'nutrisslim-suiteV2'),
            'type' => 'number',
...
function add_custom_fees_to_cart() {
    ...
    $custom_fees = [
        ...
        'rf_zavarovanje_narocila' => [
            'label' => __('Seguro de encomenda', 'nutrisslim-suiteV2'),
        ...
        'rf_priority_delivery' => [
            'label' => __('Entrega prioritária', 'nutrisslim-suiteV2'),
...
<div id="delivery-option" class="rf-custom-option">
    <h3><?php echo __('Atualize a sua entrega', 'nutrisslim-suiteV2'); ?></h3>
    ...
    <div class="delivery-option rf-option-container" style="<?php echo $is_priority_delivery_bg; ?>">
        <div class="rf-option-left">
            ...
            <label for="rf_priority_delivery">
                <div class="rf-option-title"><?php echo __('Furar a fila', 'nutrisslim-suiteV2'); ?>
                    (+<?php echo wc_price($priority_delivery); ?>)</div>
                <div class="rf-option-description">
                    <?php echo __('Escolha a embalagem de pedido prioritária para o envio mais rápido. Ao selecionar esta opção, seu pedido será processado antes dos outros.', 'nutrisslim-suiteV2'); ?>
                </div>
            </label>
        </div>
    ...
    <div class="delivery-option rf-option-container" style="<?php echo $is_checked_zavarovanje_bg; ?>">
        <div class="rf-option-left">
            ...
            <label for="rf_zavarovanje_narocila">
                <div class="rf-option-title"><?php echo __('Seguro de encomenda', 'nutrisslim-suiteV2'); ?>
                    (+<?php echo wc_price($package_insurance); ?>)</div>
                <div class="rf-option-description">
                    <?php echo __('Garanta a entrega da remessa e ofereça uma solução garantida e gratuita em caso de produtos danificados ou extraviados. Reenviaremos o pacote sem custo adicional para você.', 'nutrisslim-suiteV2'); ?>
                </div>
            </label>
        </div>
...
<div class="rf-custom-option select-shipping">
    <h3><?php echo __('Envio', 'woocommmerce'); ?></h3>
...
function cart_upsell() {
    ...
    if (!empty($items)) {
        ...
        $output .= '<h3>' . __('Não perca as melhores ofertas!', 'nutrisslim-suiteV2') . '</h3>';
        $output .= '<p>' . __('Apenas por tempo limitado.', 'nutrisslim-suiteV2') . '</p>';
        ...
        foreach ($items as $item) {
            ...
            $output .= '<button class="cart-button_swiper add-to-cart-icon" data-offer="cart_upsell" data-product-id="' . $pid . '" data-quantity="1" data-price="' . $offer_price . '" ' . $disabled . '>' . __('Avaliação verificada', 'nutrisslim-suiteV2') . '</button>';
...
function add_meta_to_order_fees($item, $fee_key, $fee, $order) {
	...
	$fee_sifrant_map = array(
        'surprise-box' => get_option('custom_surprise_box_sifrant'),
        'entrega-prioritaria' => get_option('priority_delivery_fee_sifrant'),
        'eco-friendly-packaging' => get_option('custom_eco_friendly_packaging_sifrant'),
        'seguro-de-encomenda' => get_option('custom_package_insurance_sifrant'),
        'taxa-adicional-para-pagamento-na-entrega' => 'STRL',
    );
