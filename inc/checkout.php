            ...
            <div class="rf-option-container"' . $is_colored . '>
                <div class="rf-option-left">
                    ...
                    <label for="rf_' . esc_attr($free_shipping_method->id) . '">
                        <div class="rf-option-title">' . esc_html($free_shipping_method->label) . $type . ' (' . esc_attr(__('Free', 'nutrisslim-suiteV2')) . ')</div>
                        <div class="rf-option-description"></div>
                    </label>
                </div>
            </div>';
    ...
    if (isset(WC()->session->chosen_payment_method) && WC()->session->chosen_payment_method === 'cod') {
        ...
        if (isset($payment_gateways['cod'])) {
            ...
            if ($extra_fee_inclusive && is_numeric($extra_fee_inclusive)) {
                ...
                WC()->cart->add_fee(__('Taxa adicional para pagamento na entrega', 'nutrisslim-suiteV2'), (float) $extra_fee_exclusive, true, $tax_class);
            }
        }
    }
}
...
function get_checkout_offer_ajax() {
        ...
        foreach ($items as $item) {
            ...
            echo '<img decoding="async" src="/wp-content/uploads/2024/03/Home-Ikone-NOVO_cart.svg" alt="Add to cart" style="vertical-align: middle; margin-right: 8px;">' . __('Adicionar ao carrinho', 'nutrisslim-suiteV2') . '</a>';
            echo '</div>'; // col-12
